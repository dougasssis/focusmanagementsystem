...
You can install following chrome extension for multi-self input tools:

* https://chrome.google.com/webstore/detail/google-input-tools/mclkkofklkfljcocdinagocijmpgbhab?hl=en

Now type your own self text in any `<input>` box or anywhere, and simply copy that text.

Now click `T` tool icon from the tool-box and press `ctrl+v` to paste your own self's text.

**To repeat it:**

1. Type your own self texts anywhere and make sure to copy to clipboard using `ctrl+v`
